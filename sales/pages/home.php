...
<body>
    ...
    $sql = "SELECT * FROM products WHERE products.expiration_date >= DATE(NOW())
            AND products.expiration_date <= DATE_ADD(DATE(NOW()), INTERVAL 1 MONTH)";
    $query = $db->prepare($sql);
    $query->execute();  // No need to pass any parameters since there are no placeholders in the query
    $fetch = $query->fetchAll();
    $rowcount = $query->rowCount();
?>
                        [<?php echo $rowcount; ?>] <?php echo $today; ?>
